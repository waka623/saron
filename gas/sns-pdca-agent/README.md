...
| `Learning.gs` | フィードバック(採用/却下/修正)の記録・参考例の取得・学習状況レポート(ドメイン非依存の共通基盤) |
| `CopywritingPrompt.gs` | コピーライティング初稿生成プロンプトの組み立て(チューニング対象) |
| `Copywriting.gs` | LINEでの「コピー作成」コマンド処理 |
...
| ドラフトへのフィードバック(学習ループ) | ドラフト提示直後に「採用」「却下」「修正: (最終版)」のいずれかを送る | `ナレッジログ`シートに記録し、次回同じ領域で生成する際に直近の「採用/修正」例を参考としてプロンプトに注入する。「学習状況」で採用率を確認できる |
| コピーライティング初稿生成 | 1行目に「コピー作成」、続けてお題(投稿テーマ・ターゲット・フォーマット等)を送る | PASONA/AIDA等の型に沿った投稿文・DM文言の初稿を2〜3パターン返信(学習ループにも接続済み) |
...
## 学習ループ(フィードバックでエージェントを育てる)

**注意: Claude自体のモデルが賢くなるわけではない。** 「採用/却下/修正」というフィードバックを`ナレッジログ`シート
(`Config.gs`の`KNOWLEDGE_LOG_SHEET_NAME`)に蓄積し、次回同じ領域(現状はICP設計のみ)で生成する際に、直近の
「採用」「修正」例をプロンプトへ参考として注入することで、出力がフィードバックを反映していく仕組み。

### 使い方

1. ドラフト(現状はICP仮説)が提示されたら、続けて次のいずれかを送る
   - `採用` — このまま良かった
   - `却下` — 使えなかった(理由は記録されないので、次に活かしたい場合は「修正」で最終版を送るほうが効果的)
   - `修正: (実際に使った最終版)` — 直して使った場合。次回以降、この最終版が「正解」として参照される
2. `学習状況` と送ると、領域ごとの採用/修正/却下件数と反映率を確認できる

### 設計上の注意

- ドメイン非依存の共通基盤として作ってあるので、今後コピーライティングやオファー設計を実装する際も同じ`Learning.gs`をそのまま使い回せる
- フィードバックが無い(何も送らない)場合は、単にログが増えないだけで動作に影響しない
- ナレッジログが増えすぎた場合の整理(古い却下ログの削除など)は未実装。運用しながら必要になれば追加する
