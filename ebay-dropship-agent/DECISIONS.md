...
## 利益ガードの目標利益率を20%→15%へ引き下げ(2026-09-06)

利益ガードの目標利益率を20%→15%へ引き下げ。理由: サプライヤー(TopDawg)が全商品を卸値≒MSRPの
58%で固定値付けしており、eBay手数料13%＋送料約$9を差し引くと20%は中価格帯で構造的に達成困難。
中価格帯の在庫商品を候補化するため15%に調整。純利益$5下限は維持。安全ゲートは不変。

---

## 運用ルール: 出品候補の選定基準はMSRPではなく実eBay相場(Sold価格)とする(2026-09-06、記録のみ・コード変更なし)

出品候補の選定は MSRP ではなく"実eBay相場(Sold価格)"で判定する。理由: TopDawgのMSRPは汎用品
では実売と乖離し(例: 100個入りペットボウ MSRP$89に対しeBay実売≒$33で、卸値$51.80が実売を
上回り赤字)、profit guardをMSRPで通しても現実には損失になる。今後の候補は次の5条件を全て
満たすもののみ: (1)在庫≥1 (2)ブランド/差別化品 (3)eBayで実際に売れている(Sold実績あり)
(4)実eBay相場 ≥(仕入値+送料)/0.72[利益率15%] (5)中価格帯$40〜120を優先。MSRPおよび
TopDawgの推定利益は判定に使わない。

この決定は運用ルールの記録であり、コード側の変更(`research.evaluate_candidate`が受け取る
`MarketSnapshot.median_price`の供給元をどう設定するか等)は今回のエントリでは行っていない。
現状の`MockMarketDataProvider`/固定値注入によるdry-runは、この運用ルールが求める
